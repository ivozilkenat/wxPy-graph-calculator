...
class DynamicMultiSplitter(MultiSplitterWindow):
	...
	def AppendWindow(self, window, sashPos=-1, proportion = 1):
		...
		if self._proportions is None:
			...
			self._proportions.Add([proportion], [window.GetSize() if proportion < 1 else self.Proportions.SplitterSize.NoFixedSize])
	...
	def SetProportions(self, proportions):
		...
		self._proportions = self.Proportions(self._orient, proportions, [w.GetSize() if proportions[i] < 1 else self.Proportions.SplitterSize.NoFixedSize for i, w in enumerate(self._windows)])
	...
	def __initSashGravity(self):
		self.Bind(wx.EVT_SIZE, self._onSize)  # <- how does CallAfter work exactly / maybe unecessary if self.SizeWindows may solve this
